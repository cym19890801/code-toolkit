...
public class Generator {
	 ...
	 * 是否覆盖生成
	 */
	private Boolean overrideGenerate=Boolean.parseBoolean(System.getProperty("gg.isOverride","false"));
	/**
	 * 生成的文件
	 */
	private List<File> generateFiles = new ArrayList();
	
	public Generator() {
	}

	public List<File> getGenerateFiles() {
		return generateFiles;
	}

	public void setGenerateFiles(List<File> generateFiles) {
...
//		if(outRootDir == null) throw new IllegalStateException("'outRootDir' property must be not null.");
    ...
    public Boolean getOverrideGenerate() {
        return overrideGenerate;
    }

    public void setOverrideGenerate(Boolean overrideGenerate) {
        this.overrideGenerate = overrideGenerate;
    }
	...
	private List<Exception> scanTemplatesAndProcess(File templateRootDir, Map templateModel,Map filePathModel,boolean isDelete) throws Exception {
		...
		logger.debug(MessageFormat.format("templateRootDir：{0} /n outRootDir: {1}", templateRootDir.getAbsolutePath(),new File(outRootDir).getAbsolutePath()));
		...
		for(int i = 0; i < srcFiles.size(); i++) {
			...
			try {
				...
				if (ignoreTemplateGenerateException) {
			        ...
			        logger.error(MessageFormat.format("iggnore generate error,template is: {0}; \n cause:{1}", srcFile,e.getMessage()));
			        exceptions.add(e);
			    } else {
					throw e;
			    }
	...
	private class TemplateProcessor {
		...
		private void executeGenerate(File templateRootDir,Map templateModel, Map filePathModel ,File srcFile) throws SQLException, IOException, TemplateException {
			...
			if(isCopyBinaryFile && FileHelper.isBinaryFile(srcFile)) {
				...
				logger.debug(MessageFormat.format("[copy binary file by extention] from:{0} => {1}",srcFile,new File(getOutRootDir(),outputFilepath)));
            ...
            try {
                String outputFilepath = proceeForOutputFilepath(filePathModel,templateFile);
                //System.out.println("生成文件地址："+outputFilepath);
                //logger.info(MessageFormat.format("生成文件目录 : {0} , 相对路径：  {1}",getOutRootDir(), outputFilepath));
                ...
                if(gg.isIgnoreOutput()) {
                    ...
                    logger.debug(MessageFormat.format("[not generate] by gg.isIgnoreOutput()=true on template: {0}",templateFile));
                    return;
                }
                
                if(StringHelper.isNotBlank(gg.getOutputFile())) {
                    generateNewFileOrInsertIntoFile(templateFile,gg.getOutputFile(templateFile), templateModel);
                }
			}catch(Exception e) {
...
//			gg.setOverride(getOverrideGenerate());
			
			gg.setOverride(Boolean.valueOf((String)GeneratorFacade.GeneratorContext.getContext().get(GeneratorConsts.CONFIG_OVERRIDE_GENERATE)));
		}
		...
		private void generateNewFileOrInsertIntoFile(String templateFile,String outputFilepath, Map templateModel) throws Exception {
			...
			if(absoluteOutputFilePath.exists() && !gg.isOverride()) {
				logger.warn(MessageFormat.format("输出文件已经存在，而且覆盖生成配置为:false，不再重新生成当前文件。输出文件为: {0} ", outputFilepath));
				return;
			}
			if(absoluteOutputFilePath.exists()) {
			    logger.warn(MessageFormat.format("输出文件已经存在,执行了覆盖生成。输出文件为: {0} ", outputFilepath));
			}
			logger.info(MessageFormat.format("[生成文件]模板: {0} ==> 输出文件: {1}",templateFile,outputFilepath));
			...
			// 添加生成文件
			generateFiles.add(absoluteOutputFilePath);
		}
	}
	...
	static class GeneratorHelper {
		public static boolean isIgnoreTemplateProcess(File srcFile,String templateFile,String includes,String excludes) {
			...
			if(srcFile.getName().toLowerCase().endsWith(".include")){
				...
				logger.debug(MessageFormat.format("[skip]\\t\\t endsWith '.include' template: {0}",templateFile));
				return true;
			}
