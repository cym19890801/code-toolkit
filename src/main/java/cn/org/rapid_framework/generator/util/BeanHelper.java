		...
		for(int i = 0; i < descriptors.length; i++ ) {
			...
			if (readMethod != null) {
				try {
				    ...
				    logger.error(MessageFormat.format("error get property value,name:{0} on bean:{1}", name,obj), e);
				    //throw new Exception(MessageFormat.format("error get property value,name:{0} on bean:{1}", name,obj), e);
