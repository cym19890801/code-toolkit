...
public class GeneratorControl {
	...
	/**
	 * 增加了work_space_addr逻辑，在指定生成地址的情况下，直接将代码生成到项目中
	 * @return
	 */
	public String getOutputFile(String ...templateFiles) {
	    if(outputFile != null && new File(outputFile).isAbsolute()) {
	        return outputFile;
	    }else {
	    	if(templateFiles.length==0){//默认
	    		//默认逻辑，生成到当前项目的工作区
	    		String s=new File(getOutRoot(),outputFile).getAbsolutePath();
		    	return s;
			}else{
				String s="";
				String project_addr= GeneratorProperties.getProperty("project_addr");
				if(StringHelper.isNotBlank(project_addr)){//如果工程名不为空，则直接将代码生成到该工程下
					s=new File(project_addr+getOutRoot(),outputFile).getAbsolutePath();
				}else{
					s=new File(getOutRoot(),outputFile).getAbsolutePath();
				}
		    	return s;
			}
	    }
